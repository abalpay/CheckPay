...
describe('getUpstreamUrl', () => {
  it('uses the service binding URL', () => {
    expect(getUpstreamUrl('api/reconcile/json', { BACKEND_URL: 'https://svc.internal' })).toBe('https://svc.internal/api/reconcile/json')
  })
  it('handles a trailing slash on the binding URL', () => {
    expect(getUpstreamUrl('api/reconcile/json', { BACKEND_URL: 'https://svc.internal/' })).toBe('https://svc.internal/api/reconcile/json')
  })
  it('keeps a base path on the binding URL', () => {
    expect(getUpstreamUrl('api/reconcile/json', { BACKEND_URL: 'https://svc.internal/base' })).toBe('https://svc.internal/base/api/reconcile/json')
  })
  it('falls back to FASTAPI_RECONCILE_URL', () => {
    expect(getUpstreamUrl('api/reconcile/json', { FASTAPI_RECONCILE_URL: 'http://x:9/api/reconcile' })).toBe('http://x:9/api/reconcile/json')
  })
  it('defaults to local uvicorn', () => {
    expect(getUpstreamUrl('api/reconcile/json', {})).toBe('http://localhost:8000/api/reconcile/json')
  })
